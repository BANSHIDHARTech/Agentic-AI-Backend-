...
from ..core.database import supabase, db_select
        ...
        try:
            ...
            for rule in rules:
                ...
                if confidence >= threshold and confidence > best_confidence:
                    logger.info(f"   NEW BEST MATCH! (Confidence: {confidence:.3f} >= {threshold})")
                    best_match = {
        ...
        try:
            ...
            for keyword in keywords:
                ...
                if keyword_lower in query:
                    keyword_score = 1.0
                    logger.info("       [RouterService] EXACT PHRASE MATCH! Score: 1.0")
                else:
                    ...
                    for kw_word in keyword_words:
                        ...
                        for query_word in query_words:
                            if query_word == kw_word:
                                ...
                                logger.info(f"       [RouterService] EXACT WORD MATCH: \"{kw_word}\" = \"{query_word}\" (1.0)")
                            elif kw_word in query_word or query_word in kw_word:
                                # Partial word match (substring)
                                word_score = max(word_score, 0.8)
                                logger.info(f"       [RouterService] PARTIAL WORD MATCH: \"{kw_word}\" ~ \"{query_word}\" (0.8)")
                                ...
                                logger.info(f"       [RouterService] FUZZY WORD MATCH: \"{kw_word}\" ~ \"{query_word}\" (0.5)")
                        
                        # Take the best score for this keyword word
                        best_word_score = max(best_word_score, word_score)
                        
                        if word_score == 0:
                            logger.info(f"       [RouterService] NO MATCH for keyword word: \"{kw_word}\"")
                    ...
                    if len(keyword_words) > 1:
                        ...
                        logger.info(f"       [RouterService] Multi-word keyword: {len(words_with_matches)}/{len(keyword_words)} words matched, ratio: {match_ratio:.2f}")
                    else:
                ...
                if keyword_score > 0:
                    ...
                    logger.info(f"     [RouterService] Keyword \"{keyword_lower}\" contributed: {keyword_score:.3f}")
                else:
                    logger.info(f"     [RouterService] Keyword \"{keyword_lower}\" contributed: 0.000")
            ...
            if matched_keywords > 0:
                ...
                if matched_keywords >= 2:
                    ...
                    logger.info(f"   [RouterService] Multi-match bonus: +{match_bonus:.3f} for {matched_keywords} matches")
                ...
                if match_ratio >= 0.5:
                    ...
                    logger.info(f"   [RouterService] High match ratio bonus: +{ratio_bonus:.3f} for {(match_ratio * 100):.1f}% keyword coverage")
