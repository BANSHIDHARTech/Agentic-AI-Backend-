...
from ..core.database import supabase as supabase_client, get_supabase_client
...
class ToolService:
    ...
    async def create_tool(tool_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Created tool record or None if creation failed
        """
        try:
    ...
    async def get_tool(tool_id: str) -> Dict[str, Any]:
        ...
        try:
            supabase = get_supabase_client()
            response = await supabase.table('tools').select('*').eq('id', tool_id).execute()
    ...
    async def get_all_tools() -> List[Dict[str, Any]]:
        ...
        try:
            supabase = get_supabase_client()
            response = await supabase.table('tools').select('*').eq(
                'is_active', True
    ...
    async def update_tool(tool_id: str, tool_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            supabase = get_supabase_client()
            response = await supabase.table('tools').update(validated_data).eq('id', tool_id).execute()
    ...
    async def delete_tool(tool_id: str) -> bool:
        ...
        try:
            supabase = get_supabase_client()
            response = await supabase.table('tools').delete().eq('id', tool_id).execute()
    ...
    async def get_tool_usage_stats() -> Dict[str, Any]:
        ...
        try:
            ...
            response = await supabase.table('logs').select(
