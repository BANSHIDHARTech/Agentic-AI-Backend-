...
class WorkflowStreamService:
    ...
    async def get_stream_status(cls, run_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            steps_result = supabase.from_("workflow_steps").select("*").eq("workflow_run_id", run_id).order("executed_at").execute()
            
            steps = steps_result.data if not steps_result.error else []
