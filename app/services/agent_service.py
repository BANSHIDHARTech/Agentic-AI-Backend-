...
from ..core.database import db_insert, db_update, db_select, db_delete, get_supabase_client
...
class AgentService:
    ...
    def get_all_agents(cls) -> List[Dict[str, Any]]:
        """Get all agents"""
        try:
            # Use direct Supabase query instead of db_select helper
            supabase = get_supabase_client()
            ...
            if hasattr(result, 'error') and result.error:
                print(f"Error getting agents: {result.error}")
                return []
                
            # Return empty list if no data or null
