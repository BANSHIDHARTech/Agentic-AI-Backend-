...
async def request_logger(request: Request, call_next):
    ...
    # Ensure Supabase client is initialized for API routes
    if request.url.path.startswith("/api"):
        try:
            from .database import init_supabase, supabase
            if supabase is None:
                logger.warning("Initializing Supabase client in middleware")
                init_supabase()
        except Exception as e:
            logger.error(f"Failed to initialize Supabase client in middleware: {str(e)}")
            # Continue processing the request, let the endpoint handle the error
